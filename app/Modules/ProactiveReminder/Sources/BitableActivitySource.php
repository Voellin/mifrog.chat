...
/**
 * Collects Feishu Bitable (多维表格) activity within the scan window.
 * Uses `drive files.list` ordered by EditedTime, filters to type=bitable.
 */
class BitableActivitySource implements ActivitySourceInterface
{
    ...
    public function collect(ReminderScanRequest $request, array $feishuConfig): array
    {
        ...
        while ($pages < self::MAX_PAGES && ! $stopEarly) {
            try {
                ...
                Log::debug('[ProactiveReminder] bitable_list_failed', [
                    'page' => $pages,
                    'error' => $e->getMessage(),
                ]);
                break;
            ...
            foreach ($files as $file) {
                ...
                $title = trim((string) ($file['name'] ?? '未命名多维表'));
        ...
        return [new SourceCollectionResult('bitables', $records, $items)];
    }
}
